...
def get_trending_topics():
    try:
        # Google Trends RSS 피드에서 한국 IT 트렌드 수집
        ...
        # IT 관련 키워드 필터링
        it_keywords = ["AI", "GPT", "앱", "구글", "애플", "삼성", "메타", "유튜브",
                      "스마트", "IT", "테크", "소프트웨어", "게임", "클라우드",
                      "챗봇", "로봇", "드론", "전기차", "반도체", "스타트업"]
        ...
        if it_trends:
            print("트렌드 수집 성공: " + str(len(it_trends)) + "개")
            return it_trends
        else:
            print("IT 트렌드 없음, 기본 토픽 사용")
            return fallback_topics

    except Exception as e:
        print("트렌드 수집 실패: " + str(e) + " → 기본 토픽 사용")
        return fallback_topics
...
def get_unsplash_image(topic):
    try:
        ...
        url = "https://api.unsplash.com/photos/random?query=" + encoded + "&orientation=landscape&client_id=" + unsplash_key
...
# 트렌드 + 기본 토픽 합쳐서 선택
all_topics = trending + fallback_topics
