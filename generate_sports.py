...
def generate_post(category, topic, date_str, persona):
    ...
    prompt = ("너는 네이버 블로그를 운영하는 " + persona["name"] + "야.\n"
              ...
              "6. 소제목은 딱딱한 형식 말고 구어체로.\n"
              "7. 이모지는 2~3개만, 자연스러운 위치에.\n"
              "8. 분량은 500~700자.\n"
              "9. 네이버 SEO 태그 5~7개 마지막에 추가.\n\n"
              "출력 형식:\n"
              "제목: (클릭하고 싶은 제목)\n"
              "---\n"
              "(본문)\n"
...
def send_email(posts_data, date_str):
    ...
    html = ("<html><body style='font-family:sans-serif;max-width:700px;margin:0 auto;padding:20px'>"
            ...
            "📌 썸네일 이미지가 첨부되어 있어요! 네이버 블로그 대표 이미지로 바로 올리세요.</p>")

    for i, post in enumerate(posts_data):
        color = cat_colors.get(post["category"], "#333")
        cid = "thumbnail_" + str(i)
        ...
        html += ("<div style='margin:0 0 32px;padding:24px;border:1px solid #eee;"
                 ...
                 "<p style='font-size:12px;color:#f57c00;font-weight:700;margin:0 0 8px'>📸 썸네일 이미지</p>"
                 "<img src='cid:" + cid + "' style='width:300px;border-radius:8px'></div>"
    ...
    html += ("<div style='background:#f0f7ff;border-radius:12px;padding:16px;"
             "font-size:13px;color:#555;line-height:1.8'>"
             "<strong>발행 팁 💡</strong><br>"
             "• 썸네일 이미지를 네이버 블로그 대표 이미지로 바로 업로드하세요<br>"
             "• 하루 2~3편 꾸준히 발행이 네이버 최적화에 유리해요<br>"
             "• 발행 시간은 오전 7~9시, 오후 12~1시, 저녁 7~9시 추천</div>"
             "</body></html>")
    ...
    # 썸네일 PNG 이미지 첨부
    for i, post in enumerate(posts_data):
        img_data = post["thumbnail_png"]
        img = MIMEImage(img_data, _subtype="png")
        img.add_header("Content-ID", "<thumbnail_" + str(i) + ">")
        img.add_header("Content-Disposition", "attachment",
                       filename="thumbnail_" + str(i+1) + "_" + post["category"] + ".png")
        msg.attach(img)
